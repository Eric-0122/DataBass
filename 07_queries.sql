...
SELECT a.artist_name AS "Artist Name", COUNT(song_id) AS "Accreditations"
FROM accreditation ac 
LEFT JOIN artists a ON a.artist_id=ac.artist_id
...
FROM (SELECT p.user_id, gs.genre, COUNT(gs.genre) AS genre_count
    ...
    RIGHT JOIN accreditation ac ON e.song_id = ac.song_id
...
SELECT a.artist_name AS "Artist", COUNT(e.song_id) AS "Total Songs"
FROM artists a
JOIN accreditation ac ON a.artist_id = ac.artist_id
JOIN existence e ON ac.song_id = e.song_id
GROUP BY a.artist_name
ORDER BY COUNT(ac.song_id) DESC
LIMIT 3;
...
WITH playlist_genres AS (
    ...
    JOIN accreditation ac ON e.song_id = ac.song_id
    JOIN (
